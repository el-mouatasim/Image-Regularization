function [X_out,fun_val, times, mses]=deblur_l1_CPGA(Bobs,P,center,lambda,stopcriterion,tolerance,true,pars)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=1:MAXITER
   ...
   t_old=t_new;
    ...
    Y=Y-2*alpha*itrans(conj(Sbig).*D);
    Y=real(Y);
     

    % Soft thresholding 
    D=abs(Y)-lambda*alpha;
    Y=sign(Y).*((D>0).*D);
